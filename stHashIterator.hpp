...
parseSeed(const std::vector<std::string>& seedString)
{
...
class stHashIterator
{
  ...
  public:
	...
	stHashIterator(
	    ...
	    const std::vector<std::vector<unsigned>>& seed,
	    unsigned h,
	    unsigned k)
	  : m_seq(seq)
	  , m_seed(seed)
	...
	void init()
	{
		...
		unsigned locN = 0;
		while (
		    m_pos < m_seq.length() - m_k + 1 &&
		    !NTMS64(m_seq.data() + m_pos, m_seed, m_k, m_h, m_fhVal, m_rhVal, locN, m_hVec, m_hStn))
			m_pos += locN + 1;
	...
	const bool* strandArray() const { return m_hStn; }

	/** get pointer to hash values for current k-mer */
	const uint64_t* operator*() const { return m_hVec; }

	/** test equality with another iterator */
	bool operator==(const stHashIterator& it) const { return m_pos == it.m_pos; }

	/** test inequality with another iterator */
	bool operator!=(const stHashIterator& it) const { return !(*this == it); }
