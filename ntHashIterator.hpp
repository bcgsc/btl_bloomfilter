  ...
  public:
	...
	void next()
	{
		...
		if (seedTab[(unsigned char)(m_seq.at(m_pos + m_k - 1))] == seedN) {
			...
			NTMC64(
			    m_seq.at(m_pos - 1), m_seq.at(m_pos - 1 + m_k), m_k, m_h, m_fhVal, m_rhVal, m_hVec);
	}
	...
	bool operator!=(const ntHashIterator& it) const { return !(*this == it); }
	...
	static const ntHashIterator end() { return ntHashIterator(); }
